...
{
$reg = $tra->RegistrarMaterias();
exit;
}
elseif(isset($_POST['btn-update']))
{
$reg = $tra->ActualizarMaterias();
exit;
}
?>
<!DOCTYPE html>
...
<div class="row">
<div class="col-sm-12">
<div class="page-header-title"><h4 class="pull-left page-title"><i class="fa fa-edit"></i> Gestión de Materias</h4>
<ol class="breadcrumb pull-right"><li><a href="panel">Inicio</a></li>
<li><a href="materias">Control</a></li>
...
<div class="panel-heading"><h3 class="panel-title"><i class="fa fa-edit"></i> Gestión de Materias</h3></div>
<div class="panel-body">
<div class="row">
<div class="col-sm-12 col-xs-12"> 
            <div class="box-body">

 <?php  if (isset($_GET['codmateria'])) {
      
      $reg = $tra->MateriasPorId(); ?>
      
<form class="form" method="post" action="#" name="updatemateria" id="updatemateria" data-id="<?php echo $reg[0]["codmateria"] ?>">
        
    <?php } else { ?>
        
 <form class="form" method="post" action="#" name="materias" id="materias"> 
    ...
    <?php } ?>
            ...
            <label class="control-label">Seleccione Área: <span class="symbol required"></span></label>
  <i class="fa fa-bars form-control-feedback"></i>
                              <?php if (isset($reg[0]['codarea'])) { ?>
<select name="codarea" id="codarea" class="form-control" required="required">
      ...
      for($i=0;$i<sizeof($area);$i++){
                  ?>
<option value="<?php echo $area[$i]['codarea'] ?>"<?php if ($area[$i]['codarea'] == $reg[0]['codarea']) { ?> selected="selected" <?php } ?>><?php echo $area[$i]['nomarea'] ?></option>        
                      <?php } ?>
                  </select>
                             <?php } else { ?>  
<select name="codarea" id="codarea" class="form-control" required="required">
      ...
      for($i=0;$i<sizeof($area);$i++){
                  ?>
<option value="<?php echo $area[$i]['codarea'] ?>"><?php echo $area[$i]['nomarea'] ?></option>       
                      <?php } ?>
                  </select> 
                              <?php } ?>
                              </div> 
							...
							<div class="col-md-8"> 
                               <div class="form-group has-feedback"> 
    <label class="control-label">Nombre de Materia: <span class="symbol required"></span></label> 
<input type="hidden" name="codmateria" id="codmateria" <?php if (isset($reg[0]['codmateria'])) { ?> value="<?php echo $reg[0]['codmateria']; ?>" <?php } ?>>
<input name="nommateria" class="form-control" type="text" id="nommateria" onKeyUp="this.value=this.value.toUpperCase();" <?php if (isset($reg[0]['nommateria'])) { ?> value="<?php echo $reg[0]['nommateria']; ?>"<?php } ?> placeholder="Ingrese Nombre de Materia" autocomplete="off" required="required"/>
